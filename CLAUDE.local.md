...
- **MUST NOT** 使用 IDE / 编辑器内嵌的小预览窗口（in-app preview）验证 UI —— 视窗太小、不真实，会误判布局与响应式。
- **MUST** 在真实浏览器（Chrome）里验证：`npm run dev` 起服务，用正常视口检查 —— 桌面 ≥ 1280px、移动 375px。
- **SHOULD** 改动动效 / 布局后，桌面与移动两个断点都过一遍。
